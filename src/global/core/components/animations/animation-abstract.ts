...
export abstract class AbstractAnimation implements InterfaceAnimationInternal {
  ...
  public getWebAnimations = (): any[] => this.webAnimations;
  ...
  public getDirection = (): AnimationDirection => this.forceDirectionValue ?? this._direction ?? this.parentAnimation?.getDirection() ?? 'normal';

  public getEasing = (): string => (
    this.shouldForceLinearEasing
      ? 'linear'
  ...
  public getDelay = (): number => this.forceDelayValue ?? this._delay ?? this.parentAnimation?.getDelay() ?? 0;

  public getKeyframes = (): AnimationKeyFrames => this.animationKeyframes;
