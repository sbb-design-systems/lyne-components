...
describe('sbb-calendar-enhanced', () => {
  ...
  const waitForTransition = async (): Promise<void> => {
    ...
    await waitForCondition(
      () => Array.from(element.querySelectorAll('.sbb-calendar__cell')).length > 0,
    );
  };

  const goToNextView = async (element: SbbCalendarEnhancedElement): Promise<void> => {
  ...
  describe('horizontal', () => {
    ...
    it('changes to year and month selection views', async () => {
      ...
      const dayCells = Array.from(
        element.querySelectorAll<SbbCalendarDayElement>('sbb-calendar-day'),
      );
    ...
    it('does not create horizontal scrollbar when calendar is 100% width in overflow container', async () => {
      ...
      const testElement = document.createElement('sbb-calendar-enhanced');
