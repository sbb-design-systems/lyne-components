...
- [Themes](#themes)
  Standard, lean, theme variants (off-brand, safety), font & cursor customization, CSS files, full font, dark mode

- [General information and customization](#general-information-and-customization)
  Style overrides, animations, units, focus outline, stacking, scrollbar, Sass mixins, CSS classes

- [Design Tokens](#design-tokens)
  Usage, responsive tokens, integration in CSS and JavaScript

- [Colors](#colors)
  Context colors, backgrounds, text, borders, shadows, alpha values

- [Typography](#typography)
  Fonts, text styles, legend, sub/sup elements

- [Lists](#lists)
  Standard lists, step lists, icon lists, description lists
...
Whenever the SBB font is not used, it is possible to override the font family.
The replacement font should support font weights 300 (thin), 400 (normal), and 700 (bold).

| CSS variable                   | Description                                                                                                                                  |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `--sbb-typo-font-family`       | Can be used to override the font family for all text and headings. Defaults to `SBB, Inter, "Helvetica Neue", Helvetica, Arial, sans-serif`. |
| `--sbb-letter-spacing-text`    | Can be used to override the letter spacing for texts. Defaults to 0 (except for SBB Font 0.03em).                                            |
| `--sbb-letter-spacing-heading` | Can be used to override the letter spacing for headings. Defaults to 0.                                                                      |

To avoid loading the potentially unused SBB font files that are hardcoded into our theme files,
a custom theme that skips the SBB font-faces can be built using our SCSS API:

```scss
@use '@sbb-esta/lyne-elements/theme' with (
  // 'standard', 'lean', 'safety' or 'lean-safety'
  $theme: 'standard',

  // 'SBB', 'none' (default: 'SBB')
  $font-face: 'none'
);

:root {
  // Override the font family for all texts
  --sbb-typo-font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;
}
```

Include the generated theme instead of Lyne's pre-rendered theme file.

> Note that the `off-brand-theme.css` doesn't include any font-face declarations out of the box.
> We recommend to use the `Inter` font that can be included via a link tag from [Google Fonts](https://fonts.google.com/specimen/Inter).
...
| `off-brand-theme.css`             | Contains the standard theme with primary color set to an alternative color (blue) and doesn't include any font-face declarations. |
...
| `lean-off-brand-theme.css`        | Contains the lean theme with primary color set to an alternative color (blue) and doesn't include any font-face declarations.     |
...
| `--sbb-color-6`   | Level 6 text color, currently `anthracite / platinum` (light / dark). |
...
| `--sbb-border-color-6` | Level 6 border color, currently `anthracite / platinum` (light / dark). |
