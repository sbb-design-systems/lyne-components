...
export class SbbTimetableTransportationWalk {
   ...
   */
  @Prop()
  public appearance?: InterfaceTimetableTransportationWalkAttributes['appearance'] = 'first-level';
  ...
  public render(): JSX.Element {
    ...
    const a11yDistanceText = `(${
      i18nDistance[this._currentLanguage]
    } ${a11yDistance} ${a11yMeters})`;
    ...
    return (
      <p aria-label={a11yLabel} class={`walk${appearanceClasses}`} role="text" title={a11yLabel}>
        <span class="walk__icon" innerHTML={icons['walk-small']}></span>
        <span aria-hidden="true" class="walk__duration--visual" role="presentation">
          {config.duration}'
        </span>
        {secondLevel ? (
          <span aria-hidden="true" class="walk__text--visual" role="presentation">
            {i18nWalk[this._currentLanguage]}
          </span>
        ) : (
          ''
        )}
        <span class="walk__text--visually-hidden">{a11yLabel}</span>
      </p>
    );
  }
}
