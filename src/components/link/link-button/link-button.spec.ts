...
describe('sbb-link-button', () => {
  ...
  it('renders a link-button as a span with sbb-icon', async () => {
    const root = (
      await fixture(
        html` <button>
          <sbb-link-button
  ...
  it('renders a link-button as a span by setting is-static property', async () => {
    const root = await fixture(html`
      <sbb-link-button is-static> Travelcards &amp; tickets. </sbb-link-button>
    `);

    expect(root).dom.to.be.equal(`
      <sbb-link-button dir="ltr" variant="block" is-static size="s" data-slot-names="unnamed">
        Travelcards &amp; tickets.
      </sbb-link-button>
    `);
    expect(root).shadowDom.to.be.equal(`
