...
describe('sbb-timetable-row', () => {
  ...
  describe('sbb-timetable-row with badge', () => {
    it('renders', async () => {
      const { root } = await newSpecPage({
        components: [SbbTimetableRow],
        html: `<sbb-timetable-row role="row"><sbb-card-badge slot="badge" text="test"/></sbb-timetable-row>`,
      });
      ...
      expect(root).toEqualHtml(`
        <sbb-timetable-row role="row">
          ...
          <sbb-card-badge slot="badge" text="test"></sbb-card-badge>
        </sbb-timetable-row>
        `);
    });
  });
