# lyne-cta-button



<!-- Auto Generated Below -->


## Usage

### Usage

Use it like this: `<lyne-cta-button label="My button text"></lyne-cta-button>`

This is a sample code block:
```bash
npm install
```

```javascript
const foo = "bar";
```



## Properties

| Property              | Attribute                  | Description                                      | Type                                                                                                                                                     | Default                 |
| --------------------- | -------------------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| `eventId`             | `event-id`                 | Id which is send in the click event payload      | `string`                                                                                                                                                 | `undefined`             |
| `label`               | `label`                    | Label text to show on the button                 | `string`                                                                                                                                                 | `'Default button text'` |
| `samplePropForJSDocs` | `sample-prop-for-j-s-docs` |                                                  | `string`                                                                                                                                                 | `'sample prop'`         |
| `variant`             | `variant`                  | Type of the button, like primary, secondary etc. | `"primary" \| "primary-negative" \| "secondary" \| "secondary-negative" \| "tertiary" \| "tertiary-negative" \| "transparent" \| "transparent-negative"` | `'primary'`             |


----------------------------------------------


