...
export class SbbGrid {
  ...
  public componentWillLoad(): void {
    // Check slots
    this._topProductSubscription1 = Boolean(
      this._hostElement.querySelector('[slot="top-product-subscription-1"]')
    );
    this._topProductSubscription2 = Boolean(
      this._hostElement.querySelector('[slot="top-product-subscription-2"]')
    );
    ...
    this._eightColumnsCentered = Boolean(
      this._hostElement.querySelector('[slot="eight-columns-centered"]')
    );
  }
  ...
  public render(): JSX.Element {
    ...
    return (
      <Host>
        <div {...attrs}>
          {this._topProduct1 ? <slot name="top-product-1" /> : ''}
          {this._topProduct2 ? <slot name="top-product-2" /> : ''}
          {this._topProduct3 ? <slot name="top-product-3" /> : ''}
          {this._topProduct4 ? <slot name="top-product-4" /> : ''}
          {this._topProductSubscription1 ? <slot name="top-product-subscription-1" /> : ''}
          {this._topProductSubscription2 ? <slot name="top-product-subscription-2" /> : ''}
          {this._eightColumnsCentered ? <slot name="eight-columns-centered" /> : ''}
        </div>
      </Host>
    );
  }
