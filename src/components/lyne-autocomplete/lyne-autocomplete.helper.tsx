...
export default (jsonString: string): any => {
  if (!jsonString || jsonString.length === 0) {
    console.warn('lyne-autocomplete error: the items property seems to be emtpy.');
  ...
  if (!jsonArray || jsonArray.length < 1) {
    console.warn('lyne-autocomplete error: you should pass an array of objects to the items property.');
  ...
  if (foundSomethingThatIsNotAnObject || didNotFindTextKey) {
    console.warn('lyne-autocomplete error: each object in the array must contain a text property.');
