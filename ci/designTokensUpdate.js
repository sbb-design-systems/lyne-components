...
// env variables
const { eventType, buildId } = argv;

(async () => {
  try {
    if (eventType !== 'api') {
      console.log(
        '-->> This is not a build triggered via API, therefore skipping update of @sbb-esta/lyne-design-tokens'
      );
      shell.exit(0);
    }

    console.log(
      '-->> This build is triggered via API, therefore update @sbb-esta/lyne-design-tokens'
    );
