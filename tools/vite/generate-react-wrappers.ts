...
export function generateReactWrappers(library: string, manifestPath: string): PluginOption {
    ...
    config(config) {
      ...
      for (const module of manifest.modules.filter((m) => !m.path.startsWith('core/'))) {
        for (const declaration of module.declarations?.filter(
          ...
          const reactTemplate = renderTemplate(declaration, declarations, module, exports, library);
