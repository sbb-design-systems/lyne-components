...
export default ESLintUtils.RuleCreator.withoutDocs({
    ...
    return {
      ...
      AccessorProperty(node) {
        ...
        if (propertyTypes.includes('undefined')) {
          ...
          propertyTypes.length === 2 &&
          propertyTypes.includes('boolean')
        ) {
          context.report({
            node: node.typeAnnotation!.typeAnnotation,
        ...
        if (
          !node.parent.parent.declare &&
          propertyTypes.length === 1 &&
          propertyTypes[0] === 'boolean' &&
          node.decorators.every(
            (d) =>
              d.expression.type !== 'CallExpression' ||
              (d.expression.callee as TSESTree.Identifier).name !== 'forceType' ||
              (d.expression.arguments[0] as TSESTree.Identifier).name !== 'Boolean',
          )
        ) {
          context.report({
            node,
            messageId: 'propertyTypeBooleanForceType',
            fix: (fixer) =>
              node.decorators.length
                ? fixer.insertTextAfter(node.decorators.at(-1)!, ` @forceType(Boolean) `)
                : fixer.insertTextBefore(node, ` @forceType(Boolean) `),
          });
        }
      },
    };
  ...
  meta: {
    ...
    messages: {
      ...
      propertyTypeRuleNoNull: 'Accessor properties should not use null with boolean.',
      ...
      propertyTypeBooleanForceType:
        'Boolean properties should use the @forceType(Boolean) decorator.',
    },
    fixable: 'code',
    type: 'problem',
    schema: [],
